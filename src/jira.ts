		...
		member.user.send('Your Discord username is not a valid for an email address. Please respond in 1 minute with a proper alphanumerical username.')
			.then((msg) => {
				...
				collector.on('end', (collected) => {
					if (collected.size === 0 || !valid) member.user.send('No valid username recorded, please put in a request for an email here: https://holores.atlassian.net/servicedesk/customer/portal/3, or login once again to restart the process. ');
				});
			});
		// eslint-disable-next-line no-use-before-define
